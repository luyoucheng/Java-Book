...
## HashMap中作为key的对象需不需要实现equals与HashCode方法？ 

需要，因为对放入HashMap中的对象，需要先根据他的Hash判断他所在的桶，如果有相同的Hash还有根据equals判断对象是否相同，因此不实现这两个方法无法实现判断 
...
![](https://s2.ax1x.com/2019/07/20/Zx5EWV.png)

 
TreeSet与TreeMap就是基于这个原理，存入他们中的对象必须实现comparator实现排序。


*第一参数减第二个参数是升序，第二个参数减第一个参数是降序*
