...
<!-- MarkdownTOC -->
 <!-- TOC -->
- [JVM](#jvm)auto   
   - [1. 你对Java的理解？](#1-你对java的理解)auto    
   - [2. 平台无关性如何实现？](#2-平台无关性如何实现)auto      
	   -  - [2.1 如何查看字节码（Javap）](#21-如何查看字节码javap)auto       
	   -   - [2.2 流程图](#22-流程图)auto       
	   -    - [2.3 为什么JVM不直接将源码解析成机器码去执行？](#23-为什么jvm不直接将源码解析成机器码去执行)auto    
	- [3. JVM](#3-jvm)auto        
		- [3.1 JVM虚拟机](#31-jvm虚拟机)auto       
		-  - [3.2 JVM结构](#32-jvm结构)auto       
		-   - [3.3 Jvm如何加载.class文件](#33-jvm如何加载class文件)auto    
	- [4. 反射](#4-反射)auto       
		-  - [4.1 反射的概念](#41-反射的概念)auto        
		-  - [4.2 反射的实例](#42-反射的实例)auto        
		-  - [4.3 反射的流程（类从编译到执行）](#43-反射的流程类从编译到执行)auto   
 - [5. classloader](#5-classloader)auto        
		 - - [5.1 简介](#51-简介)auto      
	  - [5.2 classloder的种类](#52-classloder的种类)auto        
	  - [5.3 classloder的加载原理](#53-classloder的加载原理)auto      
	  - [5.4 双亲委派机制](#54-双亲委派机制)auto           
			  -     - [5.4.1  为什么使用双亲委派机制？](#541--为什么使用双亲委派机制)auto  
			  -               - [5.4.2  java能不能自己写一个类叫java.lang.System](#542--java能不能自己写一个类叫javalangsystem)auto   
  - [6. Java类的装载过程](#5-java类的装载过程)auto       
	  -  - [6.1  loadClass与Class.forName 区别](#61--loadclass与classforname-区别)auto 
	    
	-	[7. JVM内存模型](#7-jvm内存模型)auto       
         - [7.1 线程私有部分](#71-线程私有部分)auto            
	         - - [7.1.1  局部变量表与操作栈的区别](#711--局部变量表与操作栈的区别)auto            	
	         - - [7.1.2  递归为什么会引发java.lang.StackOverflowErrow异常？](#712--递归为什么会引发javalangstackoverflowerrow异常)auto            
	         - - [7.1.3  虚拟机栈过多会引发java.lang.OutOfMemoryError异常？](#713--虚拟机栈过多会引发javalangoutofmemoryerror异常)auto        
	     - [7.2 线程共有部分](#72-线程共有部分)auto            
		     - - [7.2.1 方法区](#721-方法区)auto            	
		     - - [7.2.2 堆](#722-堆)auto       
		 - [7.3 JVM三大调优参数含义](#73-jvm三大调优参数含义)auto      
	  - [7.4 内存分配策略](#74-内存分配策略)auto- [UNDO](#undo)auto       
	  - [7.5 堆与栈的联系](#75-堆与栈的联系)auto       
	  -  - [7.6 堆与栈的区别](#76-堆与栈的区别)auto           
		  -   - [7.6.1 栈的动态分配与静态分配](#761-栈的动态分配与静态分配)auto        
		 - [7.7 内存之间的联系](#77-内存之间的联系)auto        
		 - - [7.8 各个内存中存放的内容](#78-各个内存中存放的内容)autoauto<!-- /TOC -->
